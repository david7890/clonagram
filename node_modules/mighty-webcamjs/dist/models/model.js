...
module.exports = function (Webcam) {
	...
	var defaults = _extends({}, params, state);

	var initialisation = true;
	var model = _atomJs2.default.setup({
	...
	model.resetState = function () {
		...
		Object.keys(model.get()).forEach(function (key) {
			var isParam = key in params;
			var isDefaultState = key in state;

			isParam || model.set(key, isDefaultState ? state[key] : undefined);
		});
	};

	model.on('use_ImageCapture_API', function (value) {
